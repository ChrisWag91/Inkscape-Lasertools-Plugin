...
################################################################################
    ...
    def area_fill (self):
        ...
        for layer in self.layers :
			if layer in self.selected_paths :
				self.set_tool(layer)
				if self.laser_beam_with<=0 : 
					self.error(_("Laser beam diameter must be > 0") % layer.get(inkex.addNS('label','inkscape')),"area_tools_diameter_error")
				
				for path in selected_paths[layer]:
					lines = []
					...
					# Zig-zag
                        r = self.laser_beam_with
                        if r<=0 :
						    self.error('Laser beam diameter must be > 0', 'error')
						    return

                        lines += [ [] ]
                        
                        i = b[0] - area_fill_shift*r
                        top = True
                        last_one = True
