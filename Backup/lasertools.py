...
lasertools_current_version = "1.7"
...
'header': """%
(Header)
(Generated by lasertools from Inkscape.)
...
def draw_text(text,x,y, group = None, style = None, font_size = 10, lasertools_tag = None) :
	if style == None : 
	...
	if lasertools_tag!=None : 
		attributes["lasertools"] = str(lasertools_tag)
...
def draw_csp(csp, stroke = "#f00", fill = "none", comment = "", width = 0.354, group = None, style = None, lasertools_tag = None) :
	if style == None : 
...
def draw_pointer(x,color = "#f00", figure = "cross", group = None, comment = "", fill=None, width = .1, size = 10., text = None, font_size=None, pointer_type=None, attrib = None) :
	...
	attrib["lasertools"] = pointer_type
	if group == None:
		group = options.self.current_layer
	if text != None	:
		if font_size == None : font_size = 7
		group = inkex.etree.SubElement( group, inkex.addNS('g','svg'), {"lasertools": pointer_type+" group"} )		
...
class Arc():
	...
	def draw(self, group, style, layer, transform, num = 0, reverse_angle = 1):
		st = P(lasertools.transform(self.st.to_list(), layer, True))
		c = P(lasertools.transform(self.c.to_list(), layer, True))
...
class Line():
	...
	def draw(self, group, style, layer, transform, num = 0, reverse_angle = 1):
		st = lasertools.transform(self.st.to_list(), layer, True)
		end = lasertools.transform(self.end.to_list(), layer, True)
	...
	def intersect(self,b) :
		if b.__class__ == Line :
			...
			if x == 0 :
				...
				lasertools.error((x,t1,t2), "warning")
	...
	def draw(self, layer, group=None, style=styles["biarc_style"]):
		...
		if group==None:
			if "preview_groups" not in dir(options.self) :
				lasertools.preview_groups = { layer: inkex.etree.SubElement( lasertools.layers[min(1,len(lasertools.layers)-1)], inkex.addNS('g','svg'), {"lasertools": "Preview group"} ) }
			elif layer not in lasertools.preview_groups :
				lasertools.preview_groups[layer] = inkex.etree.SubElement( lasertools.layers[min(1,len(lasertools.layers)-1)], inkex.addNS('g','svg'), {"lasertools": "Preview group"} )
			group = lasertools.preview_groups[layer]
		
		transform = lasertools.get_transforms(group)
		if transform != [] : 
			transform = lasertools.reverse_transform(transform)
			transform = simpletransform.formatTransform(transform)

		a,b,c = [0.,0.], [1.,0.], [0.,1.]
		k = (b[0]-a[0])*(c[1]-a[1])-(c[0]-a[0])*(b[1]-a[1])
		a,b,c = lasertools.transform(a, layer, True), lasertools.transform(b, layer, True), lasertools.transform(c, layer, True)
	...
	def test_inline(self) : 
		...
		try :
			...
			try:
				...
				options.self.error("For this function Scipy is needed. See http://www.cnc-club.ru/lasertools for details.","error")		
	...
	def draw_curve(self, curve, layer, group=None, style=styles["biarc_style"]):
		...
		if group==None:
			if "preview_groups" not in dir(self) :
				self.preview_groups = { layer: inkex.etree.SubElement( self.layers[min(1,len(self.layers)-1)], inkex.addNS('g','svg'), {"lasertools": "Preview group"} ) }
...
################################################################################
###
###		Get Lasertools info from the svg
###
################################################################################
	def get_info(self):
		self.selected_paths = {}
		...
		def recursive_search(g, layer, selected=False):
			...
			for i in items:
				...
				elif i.get('lasertools') == "Lasertools orientation group" :
				...
				elif i.get("lasertools") == "Lasertools tool definition" or i.get("lasertools") == "Lasertools tool defenition"  :
				...
				elif i.get("lasertools") == "Lasertools graffiti reference point" :
				...
				elif i.get("lasertools") == "In-out reference point group" :
					items_ = i.getchildren()
					items_.reverse()
					for j in items_ :
						if j.get("lasertools") == "In-out reference point" :
...
# xgettext:no-pango-format
	...
	def get_orientation_points(self,g):
		...
		p = None
		for i in items:
			if i.tag == inkex.addNS("g",'svg') and i.get("lasertools") == "Lasertools orientation point (2 points)":
				p2 += [i]
			if i.tag == inkex.addNS("g",'svg') and i.get("lasertools") == "Lasertools orientation point (3 points)":
				p3 += [i]
		if len(p2)==2 : p=p2 
		elif len(p3)==3 : p=p3 
		...
		for i in p :	
			point = [[],[]]	
			for  node in i :
				if node.get('lasertools') == "Lasertools orientation point arrow":
					point[0] = self.apply_transforms(node,cubicsuperpath.parsePath(node.get("d")))[0][0][1]
				if node.get('lasertools') == "Lasertools orientation point text":
	...
	def get_graffiti_reference_points(self,g):
			point = [[], '']
			for node in g :
				if node.get('lasertools') == "Lasertools graffiti reference point arrow":
					point[0] = self.apply_transforms(node,cubicsuperpath.parsePath(node.get("d")))[0][0][1]
				if node.get('lasertools') == "Lasertools graffiti reference point text":
	...
	def get_tool(self, g):
		...
		for i in g:
			#	Get parameters
			if i.get("lasertools") == "Lasertools tool background" : 
				tool["style"] = simplestyle.parseStyle(i.get("style"))
			elif i.get("lasertools") == "Lasertools tool parameter" :
				key = None
				value = None
				for j in i:
					#need to recognise old tools from ver 1.6.04
					if j.get("lasertools") == "Lasertools tool definition field name" or j.get("lasertools") == "Lasertools tool defention field name":
						key = get_text(j)
					if j.get("lasertools") == "Lasertools tool definition field value" or j.get("lasertools") == "Lasertools tool defention field value":
						value = get_text(j)
						if value == "(None)": value = ""
								...
								inkex.etree.SubElement(parent, inkex.addNS('path','svg'), 
										{
											...
											'lasertools': 'area artefact arrow',
										})
...
#
#to create it, use
#Mygroup=inkex.etree.SubElement(parent, inkex.addNS('g','svg'), {"lasertools":"My group label"})
	...
	def engraving(self) :
		...
		def draw_point((x0,y0),(x,y),w,t):
			...
			if self.options.engraving_draw_calculation_paths :
				inkex.etree.SubElement(	engraving_group, inkex.addNS('path','svg'), 
						{"lasertools": "Engraving calculation toolpath", 'style':	"fill:#ff00ff; fill-opacity:0.46; stroke:#000000; stroke-width:0.1;", inkex.addNS('cx','sodipodi'):		str(x), inkex.addNS('cy','sodipodi'):		str(y), inkex.addNS('rx','sodipodi'):	str(1), inkex.addNS('ry','sodipodi'): str(1), inkex.addNS('type','sodipodi'):	'arc'})
				#Don't draw zero radius circles
				if w:
					inkex.etree.SubElement(	engraving_group, inkex.addNS('path','svg'), 
						{"lasertools": "Engraving calculation paths", 'style':	"fill:none; fill-opacity:0.46; stroke:#000000; stroke-width:0.1;", inkex.addNS('cx','sodipodi'):		str(x),  inkex.addNS('cy','sodipodi'):		str(y),inkex.addNS('rx','sodipodi'):		str(w), inkex.addNS('ry','sodipodi'):		str(w), inkex.addNS('type','sodipodi'):	'arc'})
		...
		for layer in self.layers :
			if layer in self.selected_paths :
				...
				if self.options.engraving_draw_calculation_paths :
					gcode_3Dleft = inkex.etree.SubElement(self.my3Dlayer, inkex.addNS('g','svg'), {"lasertools":"Gcode 3D L"})
					gcode_3Dright = inkex.etree.SubElement(self.my3Dlayer, inkex.addNS('g','svg'), {"lasertools":"Gcode 3D R"})
								...
								for p in nlLT[-1]: #For last sub-path
									if p[2]: inkex.etree.SubElement(	engraving_group, inkex.addNS('path','svg'),
										...
										{ "d":	"M %f,%f L %f,%f" %(p[0][0],p[0][1],p[0][0]+p[1][0]*10,p[0][1]+p[1][1]*10),
											...
											"lasertools": "Engraving bisectors"
										})				
...
################################################################################
	def orientation(self, layer=None) :
		...
		if self.options.orientation_points_count == "graffiti" :
			...
			attr = {'lasertools': "Lasertools graffiti reference point"}
			if 	transform != [] :
				attr["transform"] = transform 
			...
			inkex.etree.SubElement(	g, inkex.addNS('path','svg'), 
				{
					...
					'lasertools': "Lasertools graffiti reference point arrow"
				})
			
			draw_text(axis,graffiti_reference_points_count*100+10,-10, group = g, lasertools_tag = "Lasertools graffiti reference point text")
			...
			attr = {"lasertools":"Lasertools orientation group"}
			if 	transform != [] :
				attr["transform"] = transform 
			...
			for i in points :
				...
				g = inkex.etree.SubElement(orientation_group, inkex.addNS('g','svg'), {'lasertools': "Lasertools orientation point (%s points)" % self.options.orientation_points_count})
				inkex.etree.SubElement(	g, inkex.addNS('path','svg'), 
					{
						...
						'lasertools': "Lasertools orientation point arrow"
					})

				draw_text("(%s; %s; %s)" % (i[0],i[1],i[2]), (si[0]+10), (-si[1]-10+doc_height), group = g, lasertools_tag = "Lasertools orientation point text")
...
G01 Z1 (going to cutting z)\n""",
		...
		tools_group = inkex.etree.SubElement(layer, inkex.addNS('g','svg'), {'lasertools': "Lasertools tool definition"})
		bg = inkex.etree.SubElement(	tools_group, inkex.addNS('path','svg'), 
					{'style':	"fill:#%s;fill-opacity:0.5;stroke:#444444; stroke-width:1px;"%colors[tool_num%len(colors)], "lasertools":"Lasertools tool background"})
		...
		for key in keys :
			g = inkex.etree.SubElement(tools_group, inkex.addNS('g','svg'), {'lasertools': "Lasertools tool parameter"})
			draw_text(key, 0, y, group = g, lasertools_tag = "Lasertools tool definition field name", font_size = 10 if key!='name' else 20)
			param = tool[key]
			if type(param)==str and re.match("^\s*$",param) : param = "(None)"
			draw_text(param, 150, y, group = g, lasertools_tag = "Lasertools tool definition field value", font_size = 10 if key!='name' else 20)
			v = str(param).split("\n")
		...
		for layer in self.layers :
			if layer in self.tools :
				if layer in tools_bounds :
					...
					g.attrib["lasertools"] = "Check tools and OP asignment"
...
################################################################################
	def help(self):
		self.error(_("""Tutorials, manuals and support can be found at\nEnglish support forum:\n	http://www.cnc-club.ru/lasertools\nand Russian support forum:\n	http://www.cnc-club.ru/lasertoolsru"""),"warning")
		return
...
################################################################################
	...
	def update(self) :
		try :
			import urllib
			f = urllib.urlopen("http://www.cnc-club.ru/lasertools_latest_version", proxies = urllib.getproxies())
			a = f.read()
			for s in a.split("\n") :
				r = re.search(r"Lasertools\s+latest\s+version\s*=\s*(.*)",s)
				if r : 
					ver = r.group(1).strip()
					if ver != lasertools_current_version :
						self.error("There is a newer version of Lasertools you can get it at: \nhttp://www.cnc-club.ru/lasertools (English version). \nhttp://www.cnc-club.ru/lasertools_ru (Russian version). ","Warning")					
					else :
						self.error("You are currently using latest stable version of Lasertools.","Warning")					
					return 
			self.error("Can not check the latest version. You can check it manualy at \nhttp://www.cnc-club.ru/lasertools (English version). \nhttp://www.cnc-club.ru/lasertools_ru (Russian version). \nCurrent version is Lasertools %s"%lasertools_current_version,"Warning")					
		except :
			self.error("Can not check the latest version. You can check it manualy at \nhttp://www.cnc-club.ru/lasertools (English version). \nhttp://www.cnc-club.ru/lasertools_ru (Russian version). \nCurrent version is Lasertools %s"%lasertools_current_version,"Warning")					
...
################################################################################
	def effect(self) :
		...
		if self.options.active_tab == '"help"' :
		...
		elif self.options.active_tab not in ['"dxfpoints"','"path-to-gcode"', '"area_fill"', '"area"', '"area_artefacts"', '"laserengraving"', '"orientation"', '"tools_library"', '"lathe"', '"offset"', '"arrangement"', '"update"', '"graffiti"', '"lathe_modify_path"', '"plasma-prepare-path"']:
			...
			# Get all Lasertools data from the scene.
			self.get_info()
			if self.options.active_tab in ['"dxfpoints"','"path-to-gcode"', '"area_fill"', '"area"', '"area_artefacts"', '"laserengraving"', '"lathe"', '"graffiti"', '"plasma-prepare-path"']:
			...
			if self.options.active_tab == '"path-to-gcode"': 
				...
				self.dxfpoints()		
			elif self.options.active_tab == '"laserengraving"': 
				self.engraving()		
