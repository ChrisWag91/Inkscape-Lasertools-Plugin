...
################################################################################
    def generate_gcode(self, curve, layer, depth):
        ...
        try :
            ...
            self.last_used_tool = None
        print_("working on curve", time.strftime("%d.%m.%Y %H:%M:%S"))
    ...
    def apply_transforms(self,g,csp):
        ...
        if trans != []:
            trans[1][2] = 0
            print_('appliing Transformation', time.strftime("%d.%m.%Y %H:%M:%S"))
            print_(trans)
    ...
    def transform(self, source_point, layer, reverse=False):
        ...
        if layer not in self.transform_matrix:
            ...
            #print_(str("I: " + str(i)))
            print_("Transform: " + str(self.layers[i]) , time.strftime("%d.%m.%Y %H:%M:%S"))
            if self.layers[i] not in self.orientation_points:
                ...
                if len(points)==3:
                    print_("Layer '%s' Orientation points: " % orientation_layer.get(inkex.addNS('label','inkscape')), time.strftime("%d.%m.%Y %H:%M:%S"))
                    for point in points:
                        print_(point)
            ...
            print_("\n Layer '%s' transformation matrixes:" % layer.get(inkex.addNS('label','inkscape')), time.strftime("%d.%m.%Y %H:%M:%S") )
...
################################################################################
    def engraving(self) :
            ...
            for jj in xrange(0,len(nlLT)) : #for every subpath of this object
                for ii in xrange(0,len(nlLT[jj])) : #for every point and line
                    if nlLT[jj][ii-1][2] : #if a point
                        ...
                        t1=get_radius_to_point(x1,y1,nx,ny,nlLT[jj][ii-1][0][0],nlLT[jj][ii-1][0][1] )
        ...
        #Check for Position of resulting path
        '''
        for layer in self.layers :
            if layer in self.selected_paths :
                for node in self.selected_paths[layer] :    
                    if node.tag == inkex.addNS('path','svg'):
                        print_("node:",node.get('d'))
            ...
            if layer in self.selected_paths :
                ...
                for node in self.selected_paths[layer] :    
                    if node.tag == inkex.addNS('path','svg'):
                        ...
                        # Data Structure
                        '''
                        print_("dimensions cspi xx: ", len(cspi[0][0][0]))
                        print_("cspi xx: ", cspi[0][0][0])
                        print_("dimensions cspi yy: ", len(cspi[0][0]))
                        print_("cspi yy: ", cspi[0][0])
                        print_("dimensions cspi x: ", len(cspi[0]))
                        print_("cspi x: ", cspi[0])
                        print_("dimensions cspi y: ", len(cspi))
                        print_("cspi:",cspi)
                        '''
...
################################################################################
    def effect(self) :
        ...
        if self.options.add_infill:        
            ...
            print_("Start filling area at", time.strftime("%d.%m.%Y %H:%M:%S"))
            self.area_fill()
            
        if self.options.add_contours: 
            self.get_info() 
            self.selected_paths = self.paths
            print_("Start building contour at", time.strftime("%d.%m.%Y %H:%M:%S"))
            self.engraving()
