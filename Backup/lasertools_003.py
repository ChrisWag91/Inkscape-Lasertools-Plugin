...
engraving_tolerance = 0.01
loft_lengths_tolerance = 0.00001
...
################################################################################
	...
	def __init__(self):
		...
		self.OptionParser.add_option("",   "--laser-feedrate",				action="store", type="float", 		dest="feed", default="600.0",						help="Feed rate in unit/min")
		self.OptionParser.add_option("",   "--rapid-feedrate",				action="store", type="float", 		dest="rapidfeed", default="1500.0",					help="Feed rate in unit/min")
		self.OptionParser.add_option("",   "--laser-on",					action="store", type="string", 		dest="laserOn", default="S100",						help="Laser on command")
		self.OptionParser.add_option("",   "--laser-off",					action="store", type="string", 		dest="laserOff", default="S1",						help="Laser off command")
...
###
################################################################################
	def generate_gcode(self, curve, layer):
		...
		laserOff, laserOn = self.options.laserOff, self.options.laserOn
		lg = 'G00'
		laserFeed = "F%s"%self.options.feed		
		current_a = 0
		go_to_safe_distance = laserOff + "\n" 
		...
		#	Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
			...
			if s[1]	== 'move':
				...
				if lg=="G00": g += laserFeed + " "+ laserOn +"(Penetrate)\n"	
				#g += "G01" +c(si[0]+[s[5][1]]) + "\n"
				g += "G01" +c(si[0]) + "\n"
				lg = 'G01'
			elif s[1] == 'arc':
				...
				if (r[0]**2 + r[1]**2)>self.options.min_arc_radius**2:
					...
					if abs(r1.mag()-r2.mag()) < 0.001 :
						g += ("G02" if s[3]<0 else "G03") + c(si[0]+[ s[5][1]+0, (s[2][0]-s[0][0]),(s[2][1]-s[0][1])  ]) + laserfeed + axis4 + "\n"
					else:
						r = (r1.mag()+r2.mag())/2
						g += ("G02" if s[3]<0 else "G03") + c(si[0]+[s[5][1]+0]) + " R%.2f" % (r) + laserFeed  + axis4 + "\n"
					lg = 'G02'
				else:					
					g += "G01" +c(si[0]+[s[5][1]]) + laserFeed + "\n"
					lg = 'G01'
		if si[1] == 'end':
			g += go_to_safe_distance + tool['gcode after path'] + "\n"
		return g
					...
					for step in range( 0,  int(math.ceil( abs((zs-mind)/self.tools[layer][0]["depth step"] )) ) ):
						...
						for key in keys:
							...
							for subcurve in curves_[key][1]:
								gcode += self.generate_gcode(subcurve, layer)
	...
	def engraving(self) :
		...
		for layer in self.layers :
			if layer in self.selected_paths :
				...
				max_distuu = self.default_tool ['diameter']/2
...
################################################################################
	def effect(self) :
		...
		if self.options.active_tab not in ['"path-to-gcode"', '"area_fill"', '"area"', '"area_artefacts"', '"laserengraving"', '"offset"', '"arrangement"']:
			...
			self.get_info()
			#if self.options.active_tab in ['"path-to-gcode"', '"area_fill"', '"area"', '"area_artefacts"', '"laserengraving"']:
